...
//   console.log("param",productId,product)
...
import Homesectioncard from "../Homesectioncard/Homesectioncard";
import { useNavigate, useParams } from "react-router-dom";
import { addToCartApi, findProductsById } from "../../../State/Product/Action";
...
export default function Productdetails() {
  ...
  //   try {
  //     const response = await axios.post(`http://localhost:5454/api/cart/add`, {
  //       productId: productId,
  ...
  // Calculate new price, discounted price, and discount percentage based on quantity
  const newPrice = price * quantity;
  const newDiscountedPrice = discountedPrice * quantity;
  ...
  // const totalPrice = items.product.price * quantity;

  // console.log(products.content);
  // let gotProduct = products.content.find((product)=> product._id === Number.parseInt(productId))
  // console.log(gotProduct);
  // dispatch(addItemToCart(setProducts))
  ...
  return (
    <div className="bg-white lg:px-20">
      <div className="pt-6">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-10 px-4pt-10">
          {/* Image gallery */}
          <div className="flex flex-col items-center">
            <div className="overflow-hidden rounded-lg max-w-[30rem] max-h-[35rem]">
              <img
            ...
            <div className="mt-4 lg:row-span-3 lg:mt-0">
              ...
              <div className="flex space-x-5 items-center text-lg lg:text-xl tracking-tight text-gray-900 mt-6">
                ...
                {/* <p className="opacity-50 line-through">
                 ...
                 <p className="opacity-50 line-through">{newDiscountedPrice}</p>
                {/* <p className="text-green-600 font-semibold">
                  {selectedProduct?.discountPersent} % off
                </p> */}
                <p className="text-green-600 font-semibold">{newDiscountPercent} % off</p>
              </div>
      ...
      <span className="py-1 px-7 border rounded-sm">{cartQuantity}</span>
      <IconButton
        onClick={() => setCartQuantity((prev) => prev + 1)}
      ...
      >
        ...
        <section className=" pt-10">
          <h1 className="py-5 text-xl font-bold">Similer Products</h1>
          <div className="flex flex-wrap space-y-5">
            {men_kurta.map((item, idx) => (
              <Homesectioncard product={item} key={idx} />
            ))}
          </div>
        </section>
      </div>
